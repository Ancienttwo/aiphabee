...
/**
 * AiphaBee BeeNote — the worker-bee insight block. The mascot (in a
 * honeycomb hexagon) delivers an AI finding in a diligent voice.
 * `honey` (light) or `navy` (dark) surface.
 */

export type BeeNotePose =
  | "insight"
  | "thinking"
  | "success"
  | "risk"
...
export interface BeeNoteProps extends Omit<HTMLAttributes<HTMLDivElement>, "title"> {
  pose?: BeeNotePose;
  basePath?: string;
  src?: string;
  title?: ReactNode;
...
function HexMascot({ src, size = 56 }: { src: string; size?: number }) {
  const border = 3;
  return (
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      ...
      >
        <img
          src={src}
          alt="AiphaBee"
          style={{ width: "92%", height: "92%", objectFit: "contain", marginBottom: "-4%" }}
        />
      </div>
    </div>
  );
