/**
 * Rich IPO workbench mock dataset + lookups (FP1), ported verbatim from the
 * design prototype `docs/AiphaBee Design System/apps/ipo-workbench/data.jsx`.
 * Illustrative mock only — the real data arrives from Codex's worker inside the
 * shared `ResponseEnvelope<T>`. Typed by `../lib/api/ipo-types`.
 */
...
export const IPOS: IpoRecord[] = [
  ...
  /* ---- 4. BY INTRODUCTION — no pool / no clawback ---- */
  {
    ...
    profile: {
      overview: '子午线房托资产组合包括粤港澳大湾区 9 处优质商业物业，出租率 94%，分派收益率约 6.2%。本次以介绍方式上市，不发行新单位、不募集资金。',
      useOfProceeds: [
...
export const IPO_BY = Object.fromEntries(IPOS.map((i) => [i.id, i] as const));

/** Lookup one IPO record by id (mock helper). */
