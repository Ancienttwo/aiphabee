...
> **Last Updated**: 2026-06-21 02:02 +08
...
`get_quote_snapshot`, `get_price_history`, and `get_corporate_actions`
scaffolds now mark six tool handlers as available for synthetic no-live lookup.
Live Serving reads, partner row access, MCP protocol tool-call integration, and
frontend surfaces remain absent.
...
| Tool handlers | Six no-live scaffolds | `resolve_security`, `get_security_profile`, `get_market_calendar`, `get_quote_snapshot`, `get_price_history`, and `get_corporate_actions` have synthetic handlers; other handlers and live data access remain absent |
...
4. The capability reports `schema_ready=true`, `rights_aware=true`,
   ...
   `handler_ready_tool_count=6`, `allow_arbitrary_sql=false`, and
...
- `resolve_security`, `get_security_profile`, `get_market_calendar`,
  `get_quote_snapshot`, `get_price_history`, and `get_corporate_actions` have
  no-live synthetic handlers.
