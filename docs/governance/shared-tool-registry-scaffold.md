...
Later `resolve_security`, `get_security_profile`, and `get_market_calendar`
scaffolds now mark three tool handlers as available for synthetic no-live
lookup. MCP/API endpoints, live Serving reads, partner row access, and frontend
...
| Tool handlers | Three no-live scaffolds | `resolve_security`, `get_security_profile`, and `get_market_calendar` have synthetic handlers; other handlers and live data access remain absent |
...
4. The capability reports `schema_ready=true`, `rights_aware=true`,
   ...
   `handler_ready_tool_count=3`, `allow_arbitrary_sql=false`, and
...
- `resolve_security`, `get_security_profile`, and `get_market_calendar` have
