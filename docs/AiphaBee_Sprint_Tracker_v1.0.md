...
| 1.3 | Web Agent Runtime + Ask + 证据卡片 | 🟦 | 6 / 10 | ☐ |
...
- [ ] Sprint 1.3 的 Agent run context scaffold、ToolLoopAgent no-model planner scaffold、pre-tool-call resolution scaffold、budget/stop policy scaffold、tool enforcement scaffold 与 numeric source guard scaffold 已建立；actual tool execution/live streaming transport、evidence-binding、答案结构、失败恢复、模型路由 live 审计与前端 Ask/证据卡片尚未实现
