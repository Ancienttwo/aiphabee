...
| 1.3 | Web Agent Runtime + Ask + 证据卡片 | 🟦 | 8 / 10 | ☐ |
...
- [ ] Sprint 1.3 的 Agent run context scaffold、ToolLoopAgent no-model planner scaffold、pre-tool-call resolution scaffold、budget/stop policy scaffold、tool enforcement scaffold、numeric source guard scaffold 与 answer/evidence contract scaffold 已建立；actual tool execution/live streaming transport、live evidence-binding、失败恢复、模型路由 live 审计与前端 Ask/证据卡片 rendering 尚未实现
