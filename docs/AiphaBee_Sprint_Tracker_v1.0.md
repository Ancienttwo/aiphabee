...
| 1.3 | Web Agent Runtime + Ask + 证据卡片 | 🟦 | 2 / 10 | ☐ |
...
- [ ] Sprint 1.3 的 Agent run context scaffold 与 ToolLoopAgent no-model planner scaffold 已建立；调用前解析、预算停止策略细化、actual tool execution/live streaming transport、evidence-binding、答案结构、失败恢复、模型路由 live 审计与前端 Ask/证据卡片尚未实现
