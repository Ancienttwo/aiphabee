...
| 1.3 | Web Agent Runtime + Ask + 证据卡片 | 🟦 | 3 / 10 | ☐ |
...
- [ ] Sprint 1.3 的 Agent run context scaffold、ToolLoopAgent no-model planner scaffold 与 pre-tool-call resolution scaffold 已建立；预算停止策略细化、actual tool execution/live streaming transport、evidence-binding、答案结构、失败恢复、模型路由 live 审计与前端 Ask/证据卡片尚未实现
