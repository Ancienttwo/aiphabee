...
| 2.4 | 订阅计费 + Workflows 深度任务 + 提醒 + 数据更正 | 🟦 | 9 / 10 | ☐ |
...
- [ ] Sprint 2.4 已建立 ACC-03 subscription lifecycle audit scaffold、ACC-04 billing-to-usage-ledger reconciliation scaffold、high-cost usage reservation/pre-debit/failure-refund scaffold、AGT-09 workflow task_id/resume/notification scaffold、deep-report workflow backend scaffold、watchlist alerts scaffold、watchlist briefings scaffold、data-correction notifications scaffold 与 MCP revocation enforcement scaffold；真实 billing provider、invoice/proration/refund preview、subscription/audit/invoice/ledger live writes、live ledger reads、refund/overage rules、live Workflows execution/checkpoint writes、watchlist briefing scheduled generation/fanout、data correction live discovery/DB writes/Queue fanout、MCP live auth middleware/credential store 与 kill switch 尚未实现
