-- Staging-only entitlement promotion for the ownership dataset. Adds
-- Web field rights for the ownership RPC on top of the existing invited-
...
--
-- Like netquity-directorate-entitlement-staging.sql, this dataset's
...
-- corporate_actions/sdi_disclosure/directorate (see
-- deploy/ingest/netquity-ownership-staging.contract.json's "rights_basis"
-- for why): ownership is fundamental-domain regulatory disclosure data
-- covered by the original Netquity collaboration rights determination, not a
-- separately-confirmed market-data rights basis. The account's own
...
-- resolveSdiDisclosure/resolveDirectorate.
...
SELECT
  ...
  'netquity-ownership-entitlement-staging:' || field_pattern
FROM fields
...
SELECT
  ...
  'netquity-ownership-entitlement-staging:workspace:' || entitlement.field_pattern
FROM aiphabee_governance.data_entitlement entitlement
WHERE entitlement.dataset = 'ownership'
...
BEGIN
  ...
  IF approved_field_count <> 6 THEN
    RAISE EXCEPTION 'ownership staging entitlement readback failed: expected 6 approved fields, found %', approved_field_count;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'non-Web ownership rights unexpectedly approved';
  END IF;
END
