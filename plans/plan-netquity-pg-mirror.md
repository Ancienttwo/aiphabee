...
- Loader (bootstrap): per table `mdb-export -H -D "%Y-%m-%d" -T "%Y-%m-%d %H:%M:%S"` piped to `psql -v ON_ERROR_STOP=1` running `begin; truncate; \copy ... with (format csv); commit;` — streaming, one txn per table, `--allow-db-write` gated, `--only <mdb>` filter, `PGCLIENTENCODING=UTF8`. NULL mapping: unquoted-empty→NULL, `""`→empty string (mdb-export default aligns with COPY csv `NULL ''`).
...
- Updater (daily): `strategies.json` maps every table to replace_all | upsert_only | window_reconcile | window_replace (+window column) per dictionary semantics; unknown table → hard error. Accepts drop dir with .mdb/.zip; UnAdjPrice2 before UnAdjPrice2H; `del_sec_*.dat` upserted into `nq_ops.del_sec`.
...
- [ ] Gate: gatekeeper review of Phase 3.
- [ ] Phase 2: PlanetScale apply (blocked on credentials + explicit user confirmation; licensed data into prod boundary).
...
- Vendor docs absent for DebtData/SCShareHold/FinReportIPO → mirror from mdb directly, strategy defaults conservative, flagged in report.
